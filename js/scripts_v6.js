...
function checkKeyPressed(robotIndex){
    document.addEventListener('keydown', function(event) {
        ...
        if(event.keyCode == 16){ //JUMP     
            options.myRobotAnimationWeights.running = 0
            options.myRobotAnimationWeights.punch = 0.0
            options.myRobotAnimationWeights.jump = 1

            if (!stillJumping) {
                myRobotSpeed = 0.5
                stillJumping = true;
                
                // Create a new timeline for the robot animation
    ...
    switch(state){
        ...
        case 'competition':
            if(countDownFinished) {
                ...
                // attachListenerPositionsToRobots();
