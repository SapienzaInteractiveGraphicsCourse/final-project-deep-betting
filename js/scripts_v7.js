...
function robotCompetitionManager(robotIndex,clips){
    for(let i = 0; i< robotList.length; i++){
        if(i != myRobotIndex){
            ...
            let result = checkIfNearObstacle(i)
            if(result.near == true){
                options.robotsCurrentSpeed[i] = 0;
            ...
            robotList[i].position.setX(robotList[i].position.x + options.robotsCurrentSpeed[i])
        }
    }
}


function myRobotCompetitionManager(clips){ //function manager of the behaviour of my robot
    controlAnimation(clips,myRobotIndex);
    robotList[myRobotIndex].position.setX(robotList[myRobotIndex].position.x + options.robotsCurrentSpeed[myRobotIndex])
    ...
    document.addEventListener('keydown', function(event) {
        ...
        if(event.keyCode == 16){ //JUMP     
            ...
            if (!stillJumping) {
                ...
                robotTl.to(robotList[myRobotIndex].position, {
                  duration: 0.5,
                  y: robotList[myRobotIndex].position.y + 10,
                  ease: "power2.inOut"
                });
...
function controlAnimation(clips,robotIndex){
    ...
    if(currentAnimationActive != 'running'){
        ...
        mixers[robotIndex].addEventListener('finished', (event) => myRobotAnimationChain(event,runningAction,punchAction,jumpAction,robotIndex));
    }else{    
        runningAction.play();
    }
}
          
function myRobotAnimationChain(e,runningAction,punchAction,jumpAction,robotIndex){
...
function setRobotInitialSpeed(){
    for(let i = 0; i < robotList.length; i++){
        if(i == myRobotIndex){
            options.robotsCurrentSpeed[i] = myRobotSpeed
        }else{
            options.robotsCurrentSpeed[i]  = Math.random() * (0.2 - 0.05) + 0.05;
        }
    }
}
...
function checkIfNearObstacle(robotIndex){
    ...
    for (let i = 0; i < obstacleInLine.length; i++) {
        if (obstacleInLine[i].position.x - robotBodyList[robotIndex].position.x  < 10) {
            result.near = true;
