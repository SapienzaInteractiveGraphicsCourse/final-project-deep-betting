...
function setRobotSpeed(robotIndex,speed){
    
    if(myRobotIndex == robotIndex){
        myRobotSpeed = speed
    }
    options.robotSpeeds[robotIndex] = speed
...
function insertOstacoli(assetLoader){
    ...
    let releaseEnabled = document.querySelector("#release-obstacle").getAttribute("enabled");
    console.log("Release mode in insertOstacoli: ",releaseMode)

    if(releaseEnabled == "false"){
        console.log("Remove highlight mesh")
...
document.querySelector('#release-mode-exit').addEventListener('click',function(){
    ...
    if(!releaseMode){
        ...
        console.log("exit release mode show initial screen",releaseMode)
    }
