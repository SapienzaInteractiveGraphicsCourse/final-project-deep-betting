...
const grass = new URL('../assets/Farm_FirstAge_Level3_Wheat.gltf', import.meta.url)
...
function robotCollisonHandler(e){
    ...
    if(index != -1){
        if(reducingSpeed == false && robotArray[myRobotIndex].speed  > 0.1){
            ...
            console.log("Speed reduction -->", robotArray[myRobotIndex].speed)
        }else{
            reducingSpeed = true
        }
    }
...
function createGrass(assetLoader, num_lines) {
    ...
    assetLoader.load(
      grass.href,
      function (gltf) {
        grassMesh = gltf.scene.children[0];
        scene.add(grassMesh);
        grassMesh.traverse(function (node) {
          if (node.isMesh) node.castShadow = true;
        });
        grassMesh.scale.set(3, 3, 3);
        grassMesh.rotation.set(0, Math.PI / 2, 0);

        if (grassMesh) {
          // create and add grass meshes
          addGrassMeshes(grassMesh, -120, 0.2, -35, 2, 51);
          addGrassMeshes(grassMesh.clone(), -120, 0.2, -19.7, 2, 51, { x: 2.7 });
          addGrassMeshes(grassMesh.clone(), -120, 0.2, -4.7, 2, 51, { x: 2.7 });
          addGrassMeshes(grassMesh.clone(), -120, 0.2, 10.5, 2, 51, { x: 2.7 });
          addGrassMeshes(grassMesh.clone(), -120, 0.2, 25.5, 2, 51, { x: 2.7 });
        }
      },
      undefined,
      function (error) {
...
function addGrassMeshes(grassMesh, startX, startY, startZ, numLayers, numColumns, options = {} ) {
    const { x = 3, y = 3, z = 3 } = options;
    grassMesh.scale.set(x, y, z);
    grassMesh.position.set(startX, startY, startZ);
    for (let i = 0; i < numLayers; i++) {
        for (let j = 0; j < numColumns; j++) {
        const grass = grassMesh.clone();
        grass.position.set(
            grass.position.x + j * 5,
            grass.position.y,
            grass.position.z + i * 10
        );
        scene.add(grass);
    }
}
